...
def maxProfit(price, start, end):
    ...
    # Iterate through the prices from end to start
    for i in range(end, start - 1, -1):
        # Initialize the maximum profit for the current day
        max_profit = 0

        # Iterate through the prices from i+1 to end
...
def longestCommonSubsequence_brute(text1, text2):
    ...
    for i in range(1, len(text1) + 1):
        for j in range(1, len(text2) + 1):
            ...
            # If the characters don't match, take the maximum length from the previous rows/columns
            else:
